...
int main() {
    ...
    for (const auto& student : studentList) {
        std::cout << "  Студент " << student.first + 1 << ": " 
                  << std::fixed << std::setprecision(2) << student.second << std::endl;
    }
    
    double thresholdForLambda = 4.0;
    std::cout << "\nСтуденты с баллом выше " << thresholdForLambda << ":\n";
    
    auto printIfAboveThreshold = [thresholdForLambda](const std::pair<int, double>& student) {
        if (student.second > thresholdForLambda) {
            std::cout << "  Студент " << student.first + 1 
                      << " (" << student.second << ")\n";
        }
    };
    ...
    for (const auto& student : studentList) {
        printIfAboveThreshold(student);
    }
    
    std::cout << "\ncounter до изменения: " << counter << std::endl;
    
    auto modifyCounter = [&counter](int x) {
        counter += x;
    };
    
    modifyCounter(10);
    std::cout << "counter после изменения: " << counter << std::endl;
